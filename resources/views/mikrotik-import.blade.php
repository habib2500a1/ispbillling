<x-app-layout>
    <x-slot name="header">{{ __('Import PPP users') }} — {{ $router->router_name }}</x-slot>
    ...
    <div class="zoom-in"
         x-data='{
            ...
            submitSelected() {
                if (!this.selected.length) { alert(@json(__("Select at least one user"))); return; }
                const form = document.getElementById("mikrotik-import-form");
                form.querySelectorAll("input.usernames-dynamic").forEach(el => el.remove());
                this.selected.forEach(name => {
                    const input = document.createElement("input");
                    input.type = "hidden";
                    input.name = "usernames[]";
                    input.value = name;
                    input.className = "usernames-dynamic";
                    form.appendChild(input);
                });
                form.querySelector("[name=create_missing]").value = this.createMissing ? "1" : "0";
                form.querySelector("[name=update_existing]").value = this.updateExisting ? "1" : "0";
                form.querySelector("[name=code_format]").value = this.codeFormat;
                form.submit();
            },
            submitOne(name) {
                this.selected = [name];
        ...
        <div class="mb-3 d-flex flex-wrap gap-2 align-items-center">
            ...
            <a href="{{ route('mikrotik.import', $router->id) }}" class="btn btn-sm btn-outline-primary">
                <i class="bi bi-arrow-repeat me-1"></i>{{ __('Reload from router') }}
            </a>
        ...
        @else
            <form id="mikrotik-import-form" method="POST" action="{{ route('mikrotik.import.store', $router->id) }}">
                @csrf
                <input type="hidden" name="create_missing" value="1">
                <input type="hidden" name="update_existing" value="1">
                <input type="hidden" name="code_format" value="prefix_sequential">
            </form>
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
                        ...
                        <div class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" id="createMissing" x-model="createMissing">
                            <label class="form-check-label small" for="createMissing">{{ __('Create new') }}</label>
                        </div>
                        <div class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" id="updateExisting" x-model="updateExisting">
                            <label class="form-check-label small" for="updateExisting">{{ __('Update existing') }}</label>
                        </div>
                        <select class="form-select form-select-sm" style="max-width:180px" x-model="codeFormat">
                            ...
                            <tbody>
                                <template x-for="secret in rows" :key="secret.name">
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <button type="button" class="btn btn-sm btn-primary" @click="submitOne(secret.name)">
