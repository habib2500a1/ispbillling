...
@if ($quote)
    ...
    @php
        $cycleDays = max(1, (int) ($quote['cycle_days'] ?? 30));
        $baseExpiryIso = ($quote['service_expires_at'] ?? null)?->toDateString();
    @endphp
    <section
        class="portal-prepay-card {{ $variant === 'bill-pay' ? 'bp-prepay-card' : '' }}"
        data-prepay-form
        data-monthly="{{ $monthly }}"
        data-current-due="{{ $currentDue }}"
        data-cycle-days="{{ $cycleDays }}"
        @if ($baseExpiryIso) data-base-expiry="{{ $baseExpiryIso }}" @endif
    >
        ...
        <form method="post" action="{{ $action }}" class="portal-prepay-card__form">
            ...
            @if ($quote['projected_expires_at'] ?? null)
                <p class="portal-prepay-card__meta">Service valid until approx. <strong data-prepay-expiry>{{ $quote['projected_expires_at']->format('M j, Y') }}</strong></p>
            @endif
            ...
            @else
                <p class="portal-prepay-card__warn">
                    Online payment is not enabled right now. Ask your ISP to enable bKash, Nagad, PipraPay, SSLCommerz, or Rocket on the bill pay page.
                </p>
            @endif
        </form>
    </section>
        ...
        @push('scripts')
            <script>
                document.querySelectorAll('[data-prepay-form]').forEach((root) => {
                    ...
                    const expiryEl = root.querySelector('[data-prepay-expiry]');
                    const cycleDays = parseInt(root.dataset.cycleDays || '30', 10);
                    const baseExpiry = root.dataset.baseExpiry || '';
                    ...
                    const render = () => {
                        ...
                        if (expiryEl && baseExpiry) {
                            const base = new Date(baseExpiry);
                            if (! Number.isNaN(base.getTime())) {
                                base.setDate(base.getDate() + cycleDays * months);
                                expiryEl.textContent = base.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
                            }
                        }
                    };
