...
<div
    ...
    x-data="{
        ...
        getRecordsInGroupOnPage(group) {
            const keys = []
            for (const checkbox of this.$root?.getElementsByClassName('fi-ta-record-checkbox') ?? []) {
                if (checkbox.dataset.group !== group) continue
                keys.push(checkbox.value)
            }
            return keys
        },
        isRecordSelected(key) { return this.selectedRecords.includes(key) },
        areRecordsSelected(keys) { return keys.length > 0 && keys.every((key) => this.isRecordSelected(key)) },
        selectRecords(keys) {
            for (const key of keys) {
                if (! this.isRecordSelected(key)) this.selectedRecords.push(key)
            }
        },
        deselectRecords(keys) {
            for (const key of keys) {
                const index = this.selectedRecords.indexOf(key)
                if (index !== -1) this.selectedRecords.splice(index, 1)
            }
        },
        ...
        isGroupCollapsed(group) { return this.collapsedGroups.includes(group) },
        ...
        resetCollapsedGroups() { this.collapsedGroups = [] },
        deselectAllRecords() { this.selectedRecords = [] },
        selectAllRecords() {},
        mountAction() {},
        mountBulkAction() {},
    }"
                        ...
                        >
                            @if ($isSelectionEnabled && (! $isReordering))
                                <x-filament-tables::selection.checkbox
                                    ...
                                    :wire:key="$this->getId() . '.table.bulk-select-page.checkbox'"
