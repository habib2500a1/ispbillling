...
<x-filament-widgets::widget>
    <div wire:poll.60s>
        <x-filament::section
            heading="ONU optical power"
            description="OLT থেকে live RX (OpticalPower) — আপনার পুরনো panel-এর মতো"
            icon="heroicon-o-light-bulb"
        >
            <x-slot name="headerEnd">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ $optical_noc_url }}" class="fi-btn fi-btn-size-sm fi-color-gray fi-btn-color-gray">
                        Optical NOC
                    </a>
                    @if ($laser_settings_url)
                        <a href="{{ $laser_settings_url }}" class="fi-btn fi-btn-size-sm fi-color-gray fi-btn-color-gray">
                            Laser thresholds
                        </a>
                    @endif
                </div>
                ...
                <div class="space-y-3">
                    ...
                    @if (! empty($snapshot['suggestions']))
                        <div class="rounded-lg border border-blue-200 bg-blue-50/80 px-4 py-3 dark:border-blue-900/50 dark:bg-blue-950/20">
                            ...
                            <p class="mt-2 text-xs text-blue-800 dark:text-blue-200">Header-এ <strong>ONU সংযুক্ত করুন</strong> → উপরের EPON port সিলেক্ট করুন।</p>
                        </div>
                    @endif
                </div>
            @else
                ...
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    OpticalPower = RX dBm · Auto-refresh every 60 seconds
                </p>
            @endif
        </x-filament::section>
    </div>
