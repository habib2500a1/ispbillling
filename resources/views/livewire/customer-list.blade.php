                ...
                <div class="{{ $routers->count() > 1 ? 'col-md-10' : 'col-md-12' }}">
                    ...
                    <div class="d-flex flex-wrap gap-1">
                        <div class="filter-group d-flex gap-1 overflow-auto pb-1">
                            ...
                            <input type="radio" class="btn-check" name="collection" id="all_active_list" autocomplete="off">
...
@push('scripts')
    <script>
        function initCustomerDataTable() {
            if (typeof window.$ === 'undefined' || typeof window.$.fn === 'undefined') return false;
            if (typeof $.fn.DataTable === 'undefined' && typeof $.fn.dataTable === 'undefined') return false;
            if (!$('.customer-table').length) return false;
            if (window.__customerTableIniting) return false;
            window.__customerTableIniting = true;
            ...
            try {
                ...
                if ($.fn.dataTable && $.fn.dataTable.isDataTable('.customer-table')) {
                    $('.customer-table').DataTable().destroy();
                }

                // Same-origin path — avoids https→http mixed-content behind reverse proxy
                var dataUrl = '/customers/data';
                ...
                var table = $('.customer-table').DataTable({
                    ...
                    ajax: {
                        ...
                        error: function (xhr, error, thrown) {
                            console.error('Customer list AJAX failed', xhr && xhr.status, error, thrown);
                            if (window.Swal) {
                                Swal.fire({
                                    icon: 'error',
                                    title: '{{ __("Customer list failed") }}',
                                    text: 'HTTP ' + (xhr && xhr.status ? xhr.status : '?') + ' — ' + (thrown || error || 'request blocked')
                                });
                            }
                        }
                    },
                ...
                $('input[name="collection"]').off('change.custdt').on('change.custdt', function() {
                    resetTableState();
                    table.ajax.reload(null, true);
                });

                $('#router_filter').off('change.custdt').on('change.custdt', function() {
                    resetTableState();
                    table.ajax.reload(null, true);
                });

                $('#reset_table').off('click.custdt').on('click.custdt', function() {
                    $('#router_filter').val('');
                ...
                window.__customerTable = table;
                return true;
            } catch (e) {
                console.error('initCustomerDataTable failed', e);
                return false;
            } finally {
                window.__customerTableIniting = false;
            }
        }

        function bootCustomerDataTable(attempt) {
            attempt = attempt || 0;
            if (initCustomerDataTable()) return;
            if (attempt < 40) {
                setTimeout(function () { bootCustomerDataTable(attempt + 1); }, 150);
            }
        }

        document.addEventListener('DOMContentLoaded', function () { bootCustomerDataTable(0); });
        document.addEventListener('livewire:navigated', function () { bootCustomerDataTable(0); });
        bootCustomerDataTable(0);
    </script>
@endpush