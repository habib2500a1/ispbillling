...
if [[ -f deploy/ensure-permissions.sh ]] && [[ -d /var/www/html ]]; then
  sh deploy/ensure-permissions.sh
  exit 0
fi
...
find storage/framework/cache -type f ! -user "${WEB_USER}" -exec chown "${WEB_USER}:${WEB_GROUP}" {} \; 2>/dev/null || true
find storage/framework/sessions -type f ! -user "${WEB_USER}" -exec chown "${WEB_USER}:${WEB_GROUP}" {} \; 2>/dev/null || true
find storage/logs -type f ! -user "${WEB_USER}" -exec chown "${WEB_USER}:${WEB_GROUP}" {} \; 2>/dev/null || true
