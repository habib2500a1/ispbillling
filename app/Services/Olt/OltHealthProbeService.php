...
final class OltHealthProbeService
{
     ...
     */
    public function probeAndPersist(Olt $olt, array $pollContext = []): array
    {
    ...
    public function resolveProfile(Olt $olt): string
    {
     ...
     * @return array<string, mixed>
     */
    public function oidsForProfile(string $profile, ?Olt $olt = null): array
    {
     ...
     * @return array<string, mixed>
     */
    private function mergeMetaOverrides(Olt $olt, array $result): array
    {
        $meta = is_array($olt->meta) ? $olt->meta : [];
     ...
     * @param  array<string, mixed>  $metrics
     */
    private function maybeNotifyOltHealth(Olt $olt, array $result): void
    {
        // Telegram alerts not ported in this subset.
    }
}
