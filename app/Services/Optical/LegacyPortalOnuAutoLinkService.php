...
/**
 * legacy portal–style auto link: MikroTik hints → OLT inventory → subscriber names on optical grid.
 */
final class LegacyPortalOnuAutoLinkService
{
    ...
    public function runAfterOltSync(int $tenantId): array
    {
        ...
        Log::info('legacy_portal_onu.auto_link_complete', array_merge(['tenant_id' => $tenantId], $stats));
    ...
    public function enrichCustomersFromMikrotikSecrets(int $tenantId): int
    {
        ...
        foreach ($servers as $server) {
            try {
                ...
                Log::warning('legacy_portal_onu.mikrotik_secrets_failed', [
                    'server_id' => $server->id,
                    'error' => $e->getMessage(),
                ]);
    ...
    private function applyMikrotikHintsToCustomer(Customer $customer, array $secret): bool
    {
        ...
        if (filled($hints['comment']) && preg_match('/client\s*code\s*:\s*(\S+)/i', (string) $hints['comment'], $m)) {
            $code = trim($m[1]);
            if ($code !== '' && blank($meta['legacy_portal_client_code_hint'] ?? null)) {
                $meta['legacy_portal_client_code_hint'] = $code;
                $changed = true;
            }
        }
    ...
    public function fetchActivePppSessionsFromRouters(int $tenantId): array
    {
        ...
        try {
            ...
            Log::warning('legacy_portal_onu.ppp_collect_failed', [
                'tenant_id' => $tenantId,
                'error' => $e->getMessage(),
            ]);
            ...
            ->chunkById(100, function ($onus) use ($tenantId, &$linked): void {
                foreach ($onus as $onu) {
                    ...
                    $candidates = array_unique(array_filter([
                        ...
                        trim((string) ($meta['aveis_equipment_id'] ?? '')),
                        trim((string) ($onu->display_name ?? '')),
                    ]));

                    $customer = null;
                    foreach ($candidates as $description) {
                        if (\App\Support\BdcomOnuDescriptionHeuristic::shouldSkipDescriptionForLinking($tenantId, $description)) {
                            continue;
                        }
