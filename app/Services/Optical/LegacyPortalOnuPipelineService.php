...
/**
 * legacy portal–style flow: OLT SNMP → inventory → PPP login match → subscriber optical panel.
 */
final class LegacyPortalOnuPipelineService
{
            ...
            if ($result['success']) {
                ...
                if ($this->fdbBridge->fdbEnabledFor($olt)) {
                    ...
                    if (! $fdb['success'] && $fdb['error'] !== null) {
                        Log::warning('legacy_portal_onu.fdb_bridge_failed', [
                            'olt_id' => $olt->id,
                            'error' => $fdb['error'],
                        ]);
                    }
                ...
                Log::warning('legacy_portal_onu.sync_olt_failed', [
                    'olt_id' => $olt->id,
    ...
    {
        $maxAgeSeconds ??= (int) config('optical.legacy_portal_inventory_max_age_seconds', 180);
