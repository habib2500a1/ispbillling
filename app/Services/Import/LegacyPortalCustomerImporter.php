...
final class LegacyPortalCustomerImporter
{
    ...
    /**
     * Resolve local package id from legacy portal row (Package + PackageSpeed).
     *
     * @param  array<string, mixed>  $row
     */
    ...
    private function buildMeta(array $row): array
    {
        return [
            ...
            'tag_free' => $this->isFreeClient($row),
            'legacy_portal_raw' => $row,
        ];
    }

    /**
     * Map legacy portal list row → local lifecycle columns (status, type, network).
     *
     * @param  array<string, mixed>  $row
     * @return array{status: string, subscriber_type: string, network_access_state: string, is_ppp_online: bool}
     */
    public function lifecycleAttributesFromRow(array $row): array
    {
        $online = $this->isPppOnline($row);
        $network = $this->mapNetworkAccessState($row);

        return [
            'status' => $this->mapStatus($row),
            'subscriber_type' => $this->resolveSubscriberType($row),
            'network_access_state' => $network,
            'is_ppp_online' => $network === 'suspended' ? false : $online,
        ];
    }
     ...
     * @param  array<string, mixed>  $row
     */
    private function resolveSubscriberType(array $row): string
    {
        if (filter_var($row['IsVIPClient'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
            return SubscriberType::VIP;
        }

        if ($this->isFreeClient($row)) {
            return SubscriberType::FREE;
        }

        return SubscriberType::STANDARD;
    }

    /**
     * legacy portal: Status "Free", ShortStatus "costfree" (dashboard: waiver / free clients).
     *
     * @param  array<string, mixed>  $row
     */
    private function isFreeClient(array $row): bool
    {
        $status = strtolower(trim((string) ($row['Status'] ?? '')));
        $short = strtolower(trim((string) ($row['ShortStatus'] ?? '')));

        return $status === 'free' || $short === 'costfree';
    }
    ...
    private function mapStatus(array $row): string
    {
        ...
        if (str_contains($status, 'suspend') || str_contains($status, 'inactive')
            || in_array($short, ['suspended', 'off', 'inactive'], true)) {
            return CustomerStatus::SUSPENDED;
        }
    ...
    private function mapNetworkAccessState(array $row): string
    {
        ...
        $short = strtolower((string) ($row['ShortStatus'] ?? 'active'));
        if (in_array($short, ['suspended', 'off', 'disabled', 'expired', 'inactive'], true)) {
            return 'suspended';
        }
    ...
    /**
     * legacy portal: EffectiveTo when set; otherwise expire day from BillingLastDate (day of month).
     *
     * @param  array<string, mixed>  $row
     */
