...
final class LegacyPortalBillingImporter
{
            ...
            foreach ($rows as $row) {
                ...
                Invoice::withoutEvents(function () use ($customer, $number, $row, $totalAmount, $paid, $issueDate, $dueDate, $force): void {
                    ...
                    $attrs = [
                        ...
                        'notes' => LegacyPortalBillNotes::serviceInvoiceNote($row['ServiceInvoiceId'] ?? '', $row['Remarks'] ?? null),
                    ];
    ...
    public function importCustomerPayments(
        ...
        $length = 200;
        do {
            $page = $client->fetchPaymentHistoryPage($customerHeaderId, $start, $length);
            $rows = $page['data'];
            $total = $page['iTotalDisplayRecords'];

            foreach ($rows as $row) {
                $stats = $this->mergePaymentRow($customer, $row, $force, $stats);
            }
        ...
        try {
            $stats = $this->mergeStats(
                $stats,
                $this->importCustomerProductInvoices($client, $customer, $customerHeaderId, $force),
            );
        } catch (\Throwable) {
            // Per-customer product invoice tab may be unavailable (HTTP 400); payment history still imported.
        }
    ...
    /**
     * @param  array{invoices: int, payments: int, skipped: int}  $stats
     * @return array{invoices: int, payments: int, skipped: int}
     */
    private function mergePaymentRow(Customer $customer, array $row, bool $force, array $stats): array
    {
        ...
        Payment::withoutEvents(function () use ($customer, $invoice, $row, $receipt, $amount, $paidAt, $force): void {
                ...
                $attrs = [
                    ...
                    'meta' => [
                        ...
                        'legacy_portal_bill_header_id' => $row['BillHeaderId'] ?? null,
                        'legacy_portal_bill_month' => $row['BillMonth'] ?? null,
    ...
    /**
     * Customer details → Service & product invoices tab.
     *
     * @return array{invoices: int, payments: int, skipped: int}
     */
    public function importCustomerProductInvoices(
        LegacyPortalSessionClient $client,
        Customer $customer,
        int $customerHeaderId,
        bool $force = false,
        ...
        do {
            $page = $client->fetchCustomerProductInvoicesPage($customerHeaderId, $start, $length);
            $rows = $page['aaData'];
            $total = $page['iTotalDisplayRecords'];

            foreach ($rows as $row) {
                $invoiceId = (string) ($row['ServiceInvoiceId'] ?? $row['InvoiceId'] ?? $row['Id'] ?? '');
                $number = 'ISD-CPI-'.$customer->customer_code.'-'.($invoiceId !== '' ? $invoiceId : md5(json_encode($row)));

                if (Invoice::query()->where('invoice_number', $number)->exists() && ! $force) {
                    $stats['skipped']++;

                    continue;
                }

                $totalAmount = $this->parseMoney($row['TotalAmount'] ?? $row['InvoiceAmount'] ?? $row['Amount'] ?? 0);
                $paid = $this->parseMoney($row['PaidAmount'] ?? $row['ReceivedAmount'] ?? 0);
                $issueDate = $this->parseDotNetDate($row['CreationDate'] ?? null)
                    ?? $this->parseFlexibleDate($row['CreationDateString'] ?? $row['InvoiceDate'] ?? null)
                    ?? now();
                $dueDate = $this->parseDotNetDate($row['PaymentDueDate'] ?? null)
                    ?? $this->parseFlexibleDate($row['DueDate'] ?? null)
                ...
                Invoice::withoutEvents(function () use ($customer, $number, $row, $totalAmount, $paid, $issueDate, $dueDate, $force): void {
                    ...
                    $attrs = [
                        ...
                        'amount_paid' => $paid,
                        'status' => $this->invoiceStatus($totalAmount, $paid),
                        'notes' => LegacyPortalBillNotes::customerInvoiceNote($row['ServiceInvoiceId'] ?? '', $row['Remarks'] ?? null),
                    ];
    ...
    public function customersByLegacyHeaderId(): Collection
    {
        ...
        Customer::query()
            ...
            ->each(function (Customer $customer) use ($map): void {
                $headerId = LegacyPortalCollectionReconcileService::resolveHeaderId($customer);
                if ($headerId === null) {
                    return;
                }
                $map->put((string) $headerId, $customer);
            });
    ...
    private function findOrCreateMonthlyInvoice(
        ...
        $existing = Invoice::query()->where('invoice_number', $number)->first();
        if ($existing !== null) {
            if ($force && $billTotal > 0 && abs((float) $existing->total - $billTotal) > 0.01) {
                $existing->updateTrusted([
                    'subtotal' => $billTotal,
                    'total' => $billTotal,
                ]);
            }
        ...
        return Invoice::withoutEvents(function () use ($customer, $number, $issueDate, $total, $billMonth): Invoice {
            return Invoice::createTrusted([
                ...
                'amount_paid' => 0,
                'status' => 'open',
                'notes' => $billMonth !== '' ? LegacyPortalBillNotes::billMonthNote($billMonth) : LegacyPortalBillNotes::importedNote(),
            ]);
        });
    }
