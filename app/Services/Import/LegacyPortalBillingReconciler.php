...
/**
 * Aligns local invoices with legacy portal billing — keeps each month's bill visible until actually paid.
 */
final class LegacyPortalBillingReconciler
{
    ...
    public function reopenConsolidatedMonthlyInvoices(?int $tenantId = null): int
    {
        ...
        $query->each(function (Invoice $invoice) use (&$reopened): void {
            ...
            $notes = trim(preg_replace('/\s*\|\s*Prior month closed — balance on current (?:legacy portal|legacy portal|'.preg_quote(\App\Support\BillingPortalLabel::name(), '/').') bill\./iu', '', $notes) ?? $notes);
