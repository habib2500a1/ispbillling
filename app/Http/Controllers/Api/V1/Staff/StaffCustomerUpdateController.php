...
class StaffCustomerUpdateController extends Controller
{
    public function update(Request $request, int $customer): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'area_id' => ['sometimes', 'integer', Rule::exists('areas', 'id')->where(fn ($q) => $q->where('tenant_id', $user->tenant_id))],
            'zone_id' => ['sometimes', 'integer', Rule::exists('zones', 'id')->where(fn ($q) => $q->where('tenant_id', $user->tenant_id))],
            'package_id' => ['sometimes', 'integer', Rule::exists('packages', 'id')->where(fn ($q) => $q->where('tenant_id', $user->tenant_id))],
