    ...
    public function userDisable()
    {
        ...
        CustomersInfo::active()
            ...
            ->each(function ($customer) use (&$successfulIDs, &$errorIDs, &$fallbackIDs, $today, $template, $tz) {
                ...
                // Per-customer grace, else site-wide disable_check_days
                $graceDays = (int) ($billing->grace_period_days ?? 0);
                if ($graceDays <= 0) {
                    $graceDays = (int) (siteUrlSettings('disable_check_days') ?? 0);
                }
                ...
                // Effective cut-off = expire date + grace period
                $autoDisableDate = $autoDisableDate->copy()->addDays($graceDays)->startOfDay();

                // for previous due
                if ($disableCheckNo <= 1) {
                    $previousDueDisableDate = Carbon::parse($billing->auto_disable_date, $tz)->month(now($tz)->month)->year(now($tz)->year)->startOfDay()->addDays($graceDays);
                } else {
                    $previousDueDisableDate = Carbon::parse($billing->auto_disable_date, $tz)->month(now($tz)->month)->year(now($tz)->year)->addDays((int) siteUrlSettings('disable_check_days') * $customer->disable_count)->startOfDay()->addDays($graceDays);
                }
