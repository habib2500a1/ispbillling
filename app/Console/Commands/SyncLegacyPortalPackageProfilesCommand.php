...
class SyncLegacyPortalPackageProfilesCommand extends Command
{
    protected $signature = 'isp:sync-package-profiles-from-legacy-portal
    ...
    protected $description = 'Set packages.mikrotik_profile_name from legacy portal PackageSpeed (display/profile)';
    ...
    public function handle(): int
    {
        ...
        $this->info("Scanning {$total} legacy portal subscribers (query={$query})…");
        ...
        foreach ($byDisplay as $entry) {
            ...
            if ($package === null) {
                ...
                if ($package !== null) {
                    $linkedByProfile++;
                    $this->line("  link #{$package->id} “{$package->name}” ← legacy portal “{$display}” / {$profile} ({$count} clients)");
                } else {
                    $missing++;
        ...
        $this->newLine();
        $this->table(['', 'Count'], [
            ['Unique package labels from legacy portal', count($byDisplay)],
