...
class NetworkSyncCustomerCommand extends Command
{
    ...
    public function handle(
        ...
        $customer = Customer::query()
            ...
            ->where(function ($q) use ($input, $digits): void {
                $numericId = is_numeric($input)
                    && $input === (string) (int) $input
                    && ! str_starts_with($input, '0');

                if ($numericId) {
                    $q->where('id', (int) $input);
                }

                $q->orWhere('customer_code', $input);

                if (ctype_digit($input) && strlen($input) < 4) {
                    $q->orWhere('customer_code', str_pad($input, 4, '0', STR_PAD_LEFT));
                }

                if ($digits !== '') {
                    $q->orWhere('phone', $digits)->orWhere('phone', $input);
                } elseif ($input !== '') {
        ...
        if ((bool) $this->option('force-mikrotik')) {
            $customer->forceFill(['network_access_state' => 'active'])->saveQuietly();
            CustomerNetworkSync::pushMikrotikApiIfAvailable($customer->fresh() ?? $customer);
            $this->info('MikroTik API push only (--force-mikrotik). Check RouterOS /ppp/secret now.');
