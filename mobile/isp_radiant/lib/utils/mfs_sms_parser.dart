...
class MfsSmsParser {
  ...
  static MfsSmsParseResult parse(String text) {
    ...
    if (body.isEmpty) return MfsSmsParseResult(skipReason: 'empty');
    ...
    if (lower.contains('bkash') || lower.contains('b-kash')) {
      ...
      gateway = 'rocket';
    } else if (RegExp(r'cash\s*in|you have received|received tk|send money', caseSensitive: false).hasMatch(body)) {
      gateway = 'bkash';
    } else {
    ...
    return MfsSmsParseResult(
      ...
      transactionId: trx,
      amount: amount,
      senderPhone: phoneMatch?.group(0),
  ...
  static String? _parseCustomerReference(String body, String trxId) {
    for (final pattern in [
      ...
      if (m != null) {
        ...
        if (token != null && token.isNotEmpty && token.toUpperCase() != trxId.toUpperCase()) {
          if (RegExp(r'^01[3-9]\d{8}$').hasMatch(token) ||
              (RegExp(r'^\d+$').hasMatch(token) && token.length >= 8)) {
            continue;
          }
          return token;
        }
  ...
  static double? _parseAmount(String body) {
    final normalized = body.replaceAllMapped(RegExp(r'(\d),(?=\d{3})'), (m) => m.group(1)!);
    for (final pattern in [
      RegExp(r'(?:TK|TAKA|BDT|AMOUNT)\s*[:\s]*([0-9]+(?:\.[0-9]{1,2})?)', caseSensitive: false),
      RegExp(r'([0-9]+(?:\.[0-9]{1,2})?)\s*(?:TK|TAKA|BDT)', caseSensitive: false),
      RegExp(r'(?:received|credited|cash\s*in)\s*(?:tk|taka|bdt)?\s*([0-9]+(?:\.[0-9]{1,2})?)', caseSensitive: false),
    ]) {
