...
class AdminTaskFragment : BaseFragment() {
    ...
    private fun load() {
        viewLifecycleOwner.lifecycleScope.launch {
            when (val res = AdminRepository().tasks()) {
    ...
    private class TaskAdapter : RecyclerView.Adapter<TaskAdapter.VH>() {
        ...
        fun submit(data: JsonArray) { items = data; notifyDataSetChanged() }
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
            VH(TextView(parent.context).apply { setPadding(32, 20, 32, 20) })
        override fun onBindViewHolder(holder: VH, position: Int) {
            val t = items[position].asJsonObject
            holder.text.text = "${t.get("title")?.asString} · ${t.get("status")?.asString}"
        }
